function [case1_confusion, case2_confusion] = GED_error_analysis( )
    ...
    for i=1:nA
        x = classA(:,i);
        AcloserB = GED(meanA, meanB, covA, covB,  x);
        if AcloserB
    ...
    for i=1: nC
        x = classC(:,i);
        CcloserD = GED(meanC, meanD, covC, covD, x);%%boolean
        CcloserE = GED(meanC, meanE, covC, covE, x);
        DcloserE = GED(meanD, meanE, covD, covE, x);
